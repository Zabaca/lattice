...
function invoke(
	...
	// The deterministic provider is pinned unless a test says otherwise: the
	// shipped default is a real local model, and the suite must neither
	// download it nor depend on it.
	return runCli({
		argv,
...
describe("hybrid search", () => {
	...
	test("refuses a corpus embedded by a different model", async () => {
		...
		// The bundle was embedded with the stub provider; the hash provider
		// can embed this query, but into a space the index knows nothing
		// about. Answering keyword-only here would quietly be a worse answer
		// to a question nobody asked, so the mismatch is refused instead.
