...
export class StatsCommand extends CommandRunner {
	...
	async run(): Promise<void> {
		try {
			const stats: GraphStats = await this.graphService.getStats();
			console.log('\n=== Graph Statistics ===\n');
...
export class RelatedCommand extends CommandRunner {
	constructor(
		private readonly graphService: GraphService,
	...
	async run(inputs: string[], options: RelatedCommandOptions): Promise<void> {
		const path = inputs[0];
