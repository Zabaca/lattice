...
lattice embed --reembed         # Rebuild the index under a changed model
```
...
With no usable embedding — no provider, or nothing in the index embedded with
this model — the search still answers from keywords alone. `--json` then
reports `"degraded": true` with a reason, so a caller can say the answer is
weaker than usual; `--require-embeddings` turns that into a non-zero exit
instead.

An index built by a *different* model is not degraded but refused: its vectors
answer a different question, and a keyword-only answer to a question nobody
asked is worse than none. The message names the old model, the new one, how
many chunks are affected, where the change came from, and
`lattice embed --reembed`.
...
| `LATTICE_EMBED_PROVIDER` | `local` (a real model), `hash` (deterministic) or `stub`. An unknown name is an error, never a silent fallback. | `local` |
...
on your own machine: `lattice init` downloads it once (~24 MB) into
`~/.lattice/models`, and every command after that works with no network. Set
`LATTICE_MODEL_DIR` to use a model directory you placed yourself, and
`HF_HUB_OFFLINE=1` to forbid downloading entirely.

`src/embed/models.ts` is the registry: where each model comes from, how it is
pooled, the prefixes it wants in front of a query as against a document, its
dimensions, and its context ceiling. The default is the cheapest of the three;
`nomic-embed-text-v1.5` reads far longer passages, and its vectors are the
only ones truncated and re-normalised, because it is the only one trained for
it. The deterministic `hash` provider remains selectable — no model on disk,
no network — but it is not semantic, so with it ranking is what keyword search
alone would give.

Changing the model cannot silently corrupt the index. `meta.embedding_model`
records which model the vectors are for; `lattice embed --reembed` writes the
new model's vectors beside the old ones and deletes the old set only in the
transaction that moves the pointer, once the new set is complete — so an
interrupted re-embed still searches correctly and simply resumes.
