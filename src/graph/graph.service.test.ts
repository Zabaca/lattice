...
describe("GraphService", () => {
	...
	describe("getStats()", () => {
		it("should return graph statistics", async () => {
			...
			expect(stats.edgeCount).toBe(50);
			expect(stats.labels).toContain("Technology");
			expect(stats.labels).toContain("Document");
			expect(typeof stats.entityCounts).toBe("object");
			expect(typeof stats.relationshipCounts).toBe("object");
		});
	});
	...
	describe("getLabels()", () => {
		it("should return all node labels", async () => {
			...
			expect(labels).toEqual(["Technology", "Document", "Concept"]);
		});

		it("should return empty array on error", async () => {
			mockRedis.setMockRejectedError(new Error("Query failed"));
	...
	describe("getRelationshipTypes()", () => {
		it("should return all relationship types", async () => {
			...
			expect(types).toEqual(["USES", "DEPENDS_ON", "DOCUMENTS"]);
		});
