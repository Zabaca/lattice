...
import { ManifestService, ChangeType } from './manifest.service.js';
...
describe('SyncService', () => {
	...
	describe('syncDocument', () => {
		it('should create Document node', async () => {
			...
			expect(mockGraph.upsertNode).toHaveBeenCalledWith('Document', expect.objectContaining({
				name: 'docs/test.md',
				title: 'Test Document',
			}));
		});
		...
		it('should create entity nodes and APPEARS_IN relationships', async () => {
			...
			expect(mockGraph.upsertNode).toHaveBeenCalledWith('Technology', expect.objectContaining({
				name: 'FalkorDB',
		...
		it('should resolve "this" in relationships to document path', async () => {
			const doc: ParsedDocument = {
				...
				relationships: [
					{ source: 'docs/falkordb-guide.md', relation: 'DOCUMENTS', target: 'FalkorDB' },
				],
				tags: [],
			};
	...
	describe('removeDocument', () => {
		it('should remove Document node', async () => {
			...
			expect(mockGraph.deleteNode).toHaveBeenCalledWith('Document', 'docs/deleted.md');
		});

		it('should remove relationships associated with document', async () => {
			await service.removeDocument('docs/deleted.md');

			expect(mockGraph.deleteDocumentRelationships).toHaveBeenCalledWith('docs/deleted.md');
		});
	});
	...
	describe('sync with force option', () => {
		...
		it('should only clear manifest entries for specified paths when force with paths', async () => {
			mockParser.discoverDocuments.mockResolvedValue(['docs/a.md', 'docs/b.md']);
			mockParser.parseDocument.mockImplementation(async (path: string) => ({
				path,
				title: path,
				content: '# Test',
