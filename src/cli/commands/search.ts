...
export async function runSearch(
	...
	try {
		// A model change is not a degraded search, it is a wrong one: the
		// vectors in the index answer a different question than the query
		// would. Refuse before ranking anything, and say how to fix it.
		const configured = tryConfiguredProvider(context.env);
		if (configured !== undefined) {
			assertModelMatches(db, configured);
		}
...
async function embedQuery(
	...
	try {
		const selected = provider ?? selectProvider(env);
		// `embedQuery`, not `embed`: a model that wants its queries marked as
		// queries must be told, or the vector lands in the wrong region.
		const vector = await selected.embedQuery(query);
		return { semantic: { vector, model: selected.model } };
	} catch (error) {
...
/**
 * The configured provider, or nothing.
 *
 * Selecting one can fail — a typo in the provider name, a home directory that
 * cannot be resolved — and that costs the semantic leg, not the command. It
 * is only when a provider IS configured that its model can disagree with the
 * index's.
 */
function tryConfiguredProvider(
	env: Record<string, string | undefined>,
